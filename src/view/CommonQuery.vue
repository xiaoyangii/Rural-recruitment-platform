    ...
    <el-tabs v-model="activeName" @tab-click="handleClick">
      <el-tab-pane label="求职进度" name="first" class="hunt">
        ...
        >
          ...
          <el-table-column prop="title" label="工作名称" width="200">
          </el-table-column>
      ...
      <el-tab-pane label="招工进度" name="second" class="recruit">
        ...
        >
          ...
          <el-table-column prop="profession" label="工作类别" width="200">
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    async fetchHuntStatus() {
      ...
      const url =
        "http://recruimentweb.vipgz4.91tunnel.com/JobHunting/selectJobHuntingByUserId";
      await request
        .get(url)
        .then(res => {
    ...
    async fetchRecruitStatus() {
      ...
      formDate.append("userId", localStorage.getItem("loginId"));
      const url = "http://recruimentweb.vipgz4.91tunnel.com/RecruitNeeds/selectNeedByUserId";
      await request
        .post(url, formDate)
        .then(res => {
          if (res.status === 200) {
            this.recruitCount = res.data.length;
            this.tableDataRecruit = res.data;
            loading.close();
            this.$message({
              message: "获取列表成功",
              type: "success"
    ...
    async EditRecruit() {
      ...
      formDate.append("title", this.editRecruitForm.title);
