      ...
      <div class="content" v-loading:#ffffff7d.scrollFixed="loading">
        ...
        <ul class="content-list">
          <li
            ...
            :key="item.recruitId"
          >
            <router-link :to="`/jobs/${item.recruitId}`">
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        industry: [
          { required: true, trigger: "blur", message: "请输入工作要求" }
        ],
      ...
      await request
        ...
        .then(res => {
          if (res.status == 200) {
            ...
            if (this.results.count !== res.data.length) {
              this.currentPage = 1;
            }
          } else {
            loading.close();
    ...
    submitForm(form) {
      this.$refs[form].validate(valid => {
        if (valid) {
          ...
          this.formDate.append("userName", this.jobForm.userName);
          this.formDate.append("userId", localStorage.getItem("loginId"));
          this.formDate.append("industry", this.jobForm.industry);
          this.formDate.append("detail", this.jobForm.detail);
          this.formDate.append("profession", this.jobForm.profession);
          const loading = this.$loading();
          this.$axios({
            url: "http://recruimentweb.vipgz4.91tunnel.com/RecruitNeeds/addNeeds",
            method: "post",
            data: this.formDate
          })
            .then(res => {
